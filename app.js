...
const App = {
    ...
    (async () => {
      try {
        ...
        if (route === 'home') {
          ...
          const filters = params?.filters ?? state.searchFilters ?? {};
          const filtersChanged = JSON.stringify(filters) !== JSON.stringify(state.searchFilters);
          if ((query && query !== state.searchQuery) || filtersChanged) {
            state.searchFilters = filters;
            state.searchResults = await API.search(query, filters);
        ...
        if (route === 'recipe' && state.currentRecipe) {
          const r = state.currentRecipe;
          document.title = `${r.strMeal} – Walkart`;
          let metaDesc = document.querySelector('meta[name="description"]');
          if (!metaDesc) { metaDesc = document.createElement('meta'); metaDesc.name = 'description'; document.head.appendChild(metaDesc); }
          metaDesc.content = `${r.strMeal} – ${r.strCategory || ''} ${r.strArea || ''} recipe on Walkart`.trim();
          ...
          ogImg.content = r.strMealThumb || '';

          // og:url + canonical
          const recipeUrl = `https://www.walkart.us/#recipe/${r.idMeal}`;
          let ogUrl = document.querySelector('meta[property="og:url"]');
          if (!ogUrl) { ogUrl = document.createElement('meta'); ogUrl.setAttribute('property','og:url'); document.head.appendChild(ogUrl); }
          ogUrl.content = recipeUrl;
          const canonical = document.getElementById('canonical-link');
          if (canonical) canonical.href = recipeUrl;
          ...
          if (!twImg) { twImg = document.createElement('meta'); twImg.name = 'twitter:image'; document.head.appendChild(twImg); }
          twImg.content = r.strMealThumb || '';

          // JSON-LD Recipe Schema → Google Rich Results
          App._injectRecipeSchema(r);
        } else {
          document.title = 'Walkart – Recettes du Monde';
          const canonical = document.getElementById('canonical-link');
          if (canonical) canonical.href = 'https://www.walkart.us/';
          App._removeRecipeSchema();
        }
        ...
        if (route === 'recipe') {
          ...
          if (recipeId && typeof FeatureRender !== 'undefined') {
            document.dispatchEvent(new CustomEvent('_recipeRendered', { detail: { id: recipeId, recipe: state.currentRecipe } }));
          }
        }
        if (route === 'home') {
          App._setupInfiniteScroll();
          // Inject async dynamic sections (recipe of day, seasonal, weather)
          const dynEl = document.getElementById('home-dynamic-sections');
          if (dynEl && typeof RecipeOfDay !== 'undefined') {
            Promise.all([
              RecipeOfDay.renderCard(),
              Promise.resolve(typeof SeasonalRecipes !== 'undefined' ? SeasonalRecipes.renderBanner() : ''),
              typeof WeatherSuggestion !== 'undefined' ? WeatherSuggestion.renderCard() : Promise.resolve('')
            ]).then(([rotd, seasonal, weather]) => {
              if (dynEl) dynEl.innerHTML = weather + seasonal + rotd;
            });
          }
        }
      } catch (e) {
  ...
  _injectRecipeSchema: (r) => {
    App._removeRecipeSchema();

    // Build ingredients array
    const ingredients = r._spoonIngredients?.length
      ? r._spoonIngredients.map(i => `${i.amount ? i.amount + ' ' : ''}${i.unit ? i.unit + ' ' : ''}${i.name}`.trim())
      : Array.from({ length: 20 }, (_, k) => k + 1)
          .map(k => r[`strIngredient${k}`]?.trim() ? `${r[`strMeasure${k}`] || ''} ${r[`strIngredient${k}`]}`.trim() : null)
          .filter(Boolean);

    // Build instructions array
    const steps = Instructions.parse(r.strInstructions);
    ...
    // Build duration ISO 8601 (PT30M)
    const cookTime = r._readyIn ? `PT${r._readyIn}M` : undefined;
    ...
    const schema = {
      ...
      description: steps[0] || `${r.strMeal} – recette ${r.strCategory || ''} ${r.strArea || ''}`.trim(),
      keywords: [r.strCategory, r.strArea, r.strTags, 'recette', 'cuisine'].filter(Boolean).join(', '),
      recipeCategory: r.strCategory || undefined,
      recipeCuisine: r.strArea || undefined,
      recipeYield: `${r._servings || 4} portions`,
      recipeIngredient: ingredients,
      recipeInstructions: instructions.length ? instructions : undefined,
      author: { '@type': 'Organization', name: 'Walkart' },
      publisher: { '@type': 'Organization', name: 'Walkart', url: 'https://walkart.us' },
      url: `https://www.walkart.us/#recipe/${r.idMeal}`,
      ...(cookTime ? { totalTime: cookTime, cookTime } : {}),
      ...(r._calories ? { nutrition: { '@type': 'NutritionInformation', calories: `${r._calories} calories` } } : {}),
      ...(rating ? { aggregateRating: { '@type': 'AggregateRating', ratingValue: rating, bestRating: 5, worstRating: 1, ratingCount: 1 } } : {}),
      ...(r.strYoutube ? { video: { '@type': 'VideoObject', name: r.strMeal, embedUrl: r.strYoutube } } : {})
    };
  ...
  _setupInfiniteScroll: () => {
    ...
    if (!sentinel) return;
    if (App._infiniteObserver) App._infiniteObserver.disconnect();
    App._infiniteObserver = new IntersectionObserver(async (entries) => {
      if (!entries[0].isIntersecting || App._infiniteLoading) return;
      App._infiniteLoading = true;
      try {
        const meals = await API.getBatch(8);
        const grid = document.getElementById('load-more-grid');
        if (grid && meals.length) {
          const cards = await Promise.all(meals.map(r => Render.recipeCard(r)));
          grid.innerHTML += cards.join('');
        }
      } catch {}
      App._infiniteLoading = false;
    }, { rootMargin: '200px' });
    App._infiniteObserver.observe(sentinel);
  },

  _setupAutocomplete: () => {
    const input = document.getElementById('header-search-input');
    if (!input || input._autocompleteSet) return;
    input._autocompleteSet = true;
    let acTimer = null;
    let acBox = document.getElementById('autocomplete-box');
    if (!acBox) {
    ...
    input.addEventListener('input', () => {
      ...
      if (q.length < 2) { acBox.innerHTML = ''; acBox.style.display = 'none'; return; }
      acTimer = setTimeout(async () => {
        try {
          const res = await fetch(`${API_BASE}/search.php?s=${encodeURIComponent(q)}`);
          const data = await res.json();
          const meals = (data.meals || []).slice(0, 6);
          if (!meals.length) { acBox.style.display = 'none'; return; }
          acBox.innerHTML = meals.map(m =>
            `<div class="ac-item" onclick="App.navigate('recipe',{id:'${Safe.attr(m.idMeal)}'});document.getElementById('autocomplete-box').style.display='none'">
              <img src="${Safe.attr(m.strMealThumb)}/preview" class="ac-thumb" loading="lazy" alt="">
              <span>${Safe.html(m.strMeal)}</span>
            </div>`
          ).join('');
    ...
    document.addEventListener('click', e => {
      if (!input.contains(e.target) && !acBox.contains(e.target)) acBox.style.display = 'none';
    }, { passive: true });
  }
};
...
const API = {
  ...
  getBatch: async (count) => {
    const supaResults = await SupaRecipes.getRandom(count);
    if (supaResults.length >= count) return supaResults;
    const needed = count - supaResults.length;
...
const SupaRecipes = {
  _norm: (r) => {
    ...
    return {
      ...
      strTags: (r.diet_tags || []).join(','),
      strSource: r.raw_data?.sourceUrl || r.raw_data?.source || '',
      strYoutube: r.raw_data?.youtube || '',
      _calories: r.calories || null,
      _protein: r.protein_g || null,
      _fat: r.fat_g || null,
      _carbs: r.carbs_g || null,
      _readyIn: r.ready_in_minutes || null,
      _servings: r.servings || 4,
      _spoonIngredients: ingredients,
      _fromSupa: true
    };
  },
  getRandom: async (count = 12) => {
    try {
      const { data } = await supa.from('recipes').select('*').limit(count);
      if (!data?.length) return [];
      const recipes = data.map(SupaRecipes._norm);
      recipes.forEach(r => RecipeStore.set(r.idMeal, r));
      return recipes;
    } catch { return []; }
  },
  search: async (query, filters = {}) => {
    try {
      let q = supa.from('recipes').select('*').limit(24);
      if (query) q = q.ilike('title', `%${query}%`);
      if (filters.maxCal) q = q.lte('calories', filters.maxCal).not('calories', 'is', null);
      if (filters.maxTime) q = q.lte('ready_in_minutes', filters.maxTime).not('ready_in_minutes', 'is', null);
      if (filters.diet) q = q.contains('diet_tags', [filters.diet]);
      const { data } = await q;
      if (!data?.length) return [];
      const recipes = data.map(SupaRecipes._norm);
      recipes.forEach(r => RecipeStore.set(r.idMeal, r));
  ...
  getById: async (id) => {
    try {
      const cached = RecipeStore.get(id);
      if (cached?._fromSupa && cached._spoonIngredients) return cached;
      const { data } = await supa.from('recipes').select('*').eq('id', id).single();
      if (!data) return null;
      const r = SupaRecipes._norm(data);
...
const CookingHistory = {
  ...
  add: (recipe) => {
    const hist = CookingHistory.get();
    hist.unshift({
      idMeal: recipe.idMeal,
      strMeal: recipe.strMeal,
      strMealThumb: recipe.strMealThumb,
      calories: recipe._calories || null,
      date: new Date().toISOString()
    });
    localStorage.setItem(CookingHistory._key, JSON.stringify(hist.slice(0, 50)));
  },
  clear: () => localStorage.removeItem(CookingHistory._key)
};

// ---- FAVORITE COLLECTIONS ----
const Collections = {
  _key: 'walkart_collections',
  get: () => JSON.parse(localStorage.getItem(Collections._key) || '{}'),
  getNames: () => Object.keys(Collections.get()),
  addCollection: (name) => {
    const c = Collections.get();
    if (!c[name]) { c[name] = []; localStorage.setItem(Collections._key, JSON.stringify(c)); }
  },
  deleteCollection: (name) => {
    const c = Collections.get();
    delete c[name];
    localStorage.setItem(Collections._key, JSON.stringify(c));
  },
  addRecipe: (colName, recipe) => {
    const c = Collections.get();
    if (!c[colName]) c[colName] = [];
    if (!c[colName].find(r => r.idMeal === recipe.idMeal)) {
      c[colName].push({ idMeal: recipe.idMeal, strMeal: recipe.strMeal, strMealThumb: recipe.strMealThumb });
    }
    localStorage.setItem(Collections._key, JSON.stringify(c));
  },
  removeRecipe: (colName, idMeal) => {
    const c = Collections.get();
    if (c[colName]) c[colName] = c[colName].filter(r => r.idMeal !== idMeal);
    localStorage.setItem(Collections._key, JSON.stringify(c));
  }
};
...
const Render = {
  ...
  home: async () => {
    ...
    return `
      ...
      <div class="container">
        ...
        <div class="discover-section">
          ...
          <div class="discover-grid">
            ...
            <div class="discover-card" onclick="App.navigate('challenges')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><path d="M24 4L29 16H42L32 24L36 37L24 29L12 37L16 24L6 16H19L24 4Z" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/></svg>
            ...
            <div class="discover-card" onclick="App.navigate('world-map')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><circle cx="24" cy="24" r="18" stroke="currentColor" stroke-width="3"/><path d="M6 24h36M24 6c-4 6-6 12-6 18s2 12 6 18M24 6c4 6 6 12 6 18s-2 12-6 18" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
              <span class="discover-label">Carte du monde</span>
            </div>
            <div class="discover-card" onclick="App.navigate('trending')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><path d="M6 36l10-12 8 6 10-14 8-10" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><path d="M34 10h8v8" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
              <span class="discover-label">Tendances</span>
            </div>
            <div class="discover-card" onclick="App.navigate('contests')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><path d="M16 6h16v16l-8 6-8-6V6z" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/><path d="M16 12H8v8c0 4 3 7 8 8M32 12h8v8c0 4-3 7-8 8" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/><line x1="24" y1="28" x2="24" y2="38" stroke="currentColor" stroke-width="3" stroke-linecap="round"/><line x1="16" y1="38" x2="32" y2="38" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg>
              <span class="discover-label">Concours</span>
            </div>
            ...
            <div class="discover-card" onclick="App.navigate('cooking-groups')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><circle cx="16" cy="18" r="6" stroke="currentColor" stroke-width="3"/><circle cx="32" cy="18" r="6" stroke="currentColor" stroke-width="3"/><path d="M4 40c0-8 5-12 12-12M44 40c0-8-5-12-12-12M20 40c0-7 2-10 4-10s4 3 4 10" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
              <span class="discover-label">Groupes</span>
            </div>
            <div class="discover-card" onclick="App.navigate('badges')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><path d="M24 4l4 10h11l-9 7 3 11-9-6-9 6 3-11-9-7h11z" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/></svg>
              <span class="discover-label">Badges</span>
            </div>
            <div class="discover-card" onclick="App.navigate('wishlist')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><path d="M24 38S8 28 8 17a8 8 0 0116-2 8 8 0 0116 2c0 11-16 21-16 21z" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/><line x1="24" y1="10" x2="24" y2="20" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/><line x1="19" y1="15" x2="29" y2="15" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
              <span class="discover-label">À essayer</span>
            </div>
            <div class="discover-card" onclick="App.navigate('cooking-history')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><circle cx="24" cy="24" r="18" stroke="currentColor" stroke-width="3"/><path d="M24 14v10l6 6" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg>
              <span class="discover-label">Historique</span>
            </div>
            ...
            <div class="discover-card" onclick="App.navigate('kitchen-stock')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><rect x="8" y="20" width="32" height="24" rx="3" stroke="currentColor" stroke-width="3"/><path d="M16 20v-6a8 8 0 0116 0v6" stroke="currentColor" stroke-width="3" stroke-linecap="round"/><line x1="24" y1="28" x2="24" y2="36" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/><line x1="20" y1="32" x2="28" y2="32" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
              <span class="discover-label">Stock cuisine</span>
            </div>
            <div class="discover-card" onclick="App.navigate('meal-budget')">
              <svg class="discover-svg" viewBox="0 0 48 48" fill="none"><rect x="6" y="14" width="36" height="24" rx="4" stroke="currentColor" stroke-width="3"/><circle cx="24" cy="26" r="5" stroke="currentColor" stroke-width="2.5"/><line x1="6" y1="20" x2="42" y2="20" stroke="currentColor" stroke-width="2"/></svg>
              <span class="discover-label">Budget repas</span>
            </div>
        ...
        <div id="infinite-sentinel" class="infinite-sentinel">
  ...
  searchResults: async (results, query) => {
    ...
    const filtered = results.filter(r => {
      if (state.searchFilter && r.strCategory !== state.searchFilter) return false;
      if (f.maxCal && r._calories && r._calories > f.maxCal) return false;
      if (f.maxTime && r._readyIn && r._readyIn > f.maxTime) return false;
      return true;
    });

    const t_results = T('search_results'), t_empty = T('no_results'),
      t_explore = T('explore_categories'), t_all = T('filter_all'),
      t_filters = T('adv_filters'), t_diet = T('filter_diet'),
      t_cal = T('filter_calories'), t_time = T('filter_time'),
      t_apply = T('filter_apply'), t_reset = T('filter_reset');

    const categories = [...new Set(results.map(r => r.strCategory).filter(Boolean))];
    const catBar = categories.length > 1 ? `
    ...
    const hasActiveFilters = f.maxCal || f.maxTime || f.diet;
    const advFilters = `
      <div class="adv-filter-bar">
        <button class="btn btn-ghost adv-filter-toggle ${hasActiveFilters ? 'filter-active' : ''}" onclick="Actions.toggleFilterPanel()">
          🎛️ ${Safe.html(t_filters)} ${hasActiveFilters ? '●' : ''}
        </button>
        <div id="filter-panel" class="filter-panel" style="display:none;">
          <div class="filter-row">
            <label>${Safe.html(t_diet)}</label>
            <select id="f-diet" class="form-input" style="max-width:160px;">
              <option value="">${Safe.html(t_all)}</option>
              <option value="vegetarian" ${f.diet==='vegetarian'?'selected':''}>🥗 Végétarien</option>
              <option value="vegan" ${f.diet==='vegan'?'selected':''}>🌱 Vegan</option>
              <option value="gluten-free" ${f.diet==='gluten-free'?'selected':''}>🌾 Sans gluten</option>
              <option value="dairy-free" ${f.diet==='dairy-free'?'selected':''}>🥛 Sans lactose</option>
              <option value="healthy" ${f.diet==='healthy'?'selected':''}>💚 Healthy</option>
            </select>
          </div>
          <div class="filter-row">
            <label>${Safe.html(t_cal)}</label>
            <input type="number" id="f-cal" class="form-input" style="max-width:100px;" placeholder="ex: 500" value="${f.maxCal || ''}">
          </div>
          <div class="filter-row">
            <label>${Safe.html(t_time)}</label>
            <input type="number" id="f-time" class="form-input" style="max-width:100px;" placeholder="ex: 30" value="${f.maxTime || ''}">
          </div>
      ...
      <div class="page-enter">
        ...
        <div class="detail-content">
          <div class="detail-card">
            <div class="detail-header">
              ...
              <div class="actions-drawer" id="recipe-actions-drawer">
                <div class="actions-drawer-grid">
                  ...
                  <button class="drawer-action-btn" onclick="Actions.shareWhatsApp('${r.idMeal}')">
                    <span class="drawer-icon">💬</span>
                    <span>${Safe.html(T('share_whatsapp') || 'WhatsApp')}</span>
                  </button>
                  <button class="drawer-action-btn" onclick="Actions.addToCollection('${r.idMeal}')">
                    <span class="drawer-icon">📁</span>
                    <span>${Safe.html(T('my_collections') || 'Collections')}</span>
                  </button>
            ...
            ${(() => {
              if (!r.strYoutube) return '';
              const ytId = r.strYoutube.match(/(?:v=|youtu\.be\/)([A-Za-z0-9_-]{11})/)?.[1];
              if (!ytId) return '';
              return `<div class="yt-embed-wrap">
                <iframe class="yt-embed" src="https://www.youtube-nocookie.com/embed/${Safe.attr(ytId)}" frameborder="0" allowfullscreen loading="lazy" title="${Safe.attr(r.strMeal)}"></iframe>
              </div>`;
            })()}
  ...
  profile: async () => {
    ...
    <div class="container" style="padding:32px 20px; max-width:600px; margin:0 auto;">
      ...
      ${(() => {
        ...
        const todayItems = history.filter(h => h.cookedAt?.startsWith(today));
        const items = history.slice(0, 5).map(h => `
          <div class="history-item" onclick="App.navigate('recipe', {id:'${Safe.attr(h.idMeal)}'})">
            <img src="${Safe.attr(h.strMealThumb || '')}" class="history-thumb" loading="lazy" alt="${Safe.attr(h.strMeal || '')}">
            <div class="history-info">
              <span class="history-name">${Safe.html(h.strMeal || '')}</span>
              <span class="history-date">${h.cookedAt ? new Date(h.cookedAt).toLocaleDateString() : ''}</span>
            </div>
          </div>`).join('');
        return `
        <div style="margin-top:24px;">
          <h3 style="font-size:1rem; font-weight:700; margin-bottom:12px;">
            🍳 ${T('cooking_history') || 'Cooking History'}
            ${todayItems.length ? `<span class="badge-cal" style="margin-left:8px;">${todayItems.length} ${T('cooked_today') || 'today'}</span>` : ''}
          </h3>
  ...
  collections: () => {
    ...
    const t_empty = T('no_history') || 'No collections yet';
    const t_title = T('my_collections') || 'My Collections';
    if (!names.length) {
      return `<div class="container" style="padding:40px 20px; text-align:center;">
        <button class="btn btn-ghost" onclick="App.goBack()" style="margin-bottom:24px;">${T('btn_back')}</button>
        <h1>📁 ${Safe.html(t_title)}</h1>
        <p style="color:var(--text-muted); margin-top:20px;">${Safe.html(t_empty)}</p>
      </div>`;
    }
    const colCards = names.map(name => {
      const recipes = cols[name];
      const thumb = recipes[0]?.strMealThumb || '';
      return `<div class="collection-card" onclick="App.navigate('collection-detail', {name:'${Safe.attr(name)}'})">
        ${thumb ? `<img src="${Safe.attr(thumb)}" class="collection-thumb" loading="lazy" alt="${Safe.attr(name)}">` : '<div class="collection-thumb-placeholder">📁</div>'}
        <div class="collection-info">
          <span class="collection-name">${Safe.html(name)}</span>
          <span class="collection-count">${recipes.length} recipe${recipes.length !== 1 ? 's' : ''}</span>
        </div>
        <button class="btn btn-ghost collection-delete" onclick="event.stopPropagation();Actions.deleteCollection('${Safe.attr(name)}')" title="Delete">🗑️</button>
      </div>`;
    }).join('');
    return `<div class="container" style="padding:20px;">
...
const Actions = {
  ...
  startCookingFromState: async () => {
    ...
    if (state.currentRecipe) CookingHistory.add(state.currentRecipe);
  ...
  applyFilters: (query) => {
    ...
    const maxCal = parseInt(document.getElementById('f-cal')?.value) || 0;
    const maxTime = parseInt(document.getElementById('f-time')?.value) || 0;
    state.searchFilters = { diet, maxCal, maxTime };
    App.navigate('search', { query, filters: state.searchFilters });
  },
  ...
  shareWhatsApp: (id) => {
    ...
    const origin = window.location.origin + window.location.pathname.replace(/\/$/, '');
    const url = `${origin}#recipe/${id}`;
    const text = encodeURIComponent(`🍽️ ${name}\n${url}`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
  },

  addToCollection: async (recipeId) => {
    const recipe = RecipeStore.get(recipeId) || state.currentRecipe;
    if (!recipe) return;
    const names = Collections.getNames();
    const [t_col, t_new, t_cancel, t_add, t_name] = await Translator.all(
      'Add to collection', 'New collection', 'Cancel', 'Add', 'Collection name'
    );
    const existing = names.map(n =>
      `<button class="btn btn-ghost" style="width:100%;margin-bottom:6px;" onclick="Actions._doAddToCollection('${recipeId}','${Safe.attr(n)}');document.getElementById('col-modal').remove()">${Safe.html(n)}</button>`
    ).join('');
    ...
    modal.innerHTML = `
      <div class="planner-modal-content" style="max-width:340px;width:92%;">
        <div class="planner-modal-header">
          <h3>📁 ${Safe.html(t_col)}</h3>
          <button class="btn btn-ghost" onclick="document.getElementById('col-modal').remove()">✕</button>
        </div>
        ${existing}
        <div style="border-top:1px solid var(--border);margin-top:8px;padding-top:8px;">
          <input id="col-new-name" class="form-input" placeholder="${Safe.attr(t_name)}" style="margin-bottom:8px;">
          <button class="btn btn-primary" style="width:100%;" onclick="
            const n=document.getElementById('col-new-name').value.trim();
            if(n){Actions._doAddToCollection('${recipeId}',n);document.getElementById('col-modal').remove();}
          ">${Safe.html(t_add)}</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
  },

  _doAddToCollection: (recipeId, collectionName) => {
    const recipe = RecipeStore.get(recipeId) || state.currentRecipe;
    if (!recipe) return;
    const added = Collections.addRecipe(collectionName, {
      idMeal: recipe.idMeal,
      strMeal: recipe.strMeal,
      strMealThumb: recipe.strMealThumb
    });
    Toast.show(added ? `📁 ${collectionName}` : '✓ Already in collection');
  },

  viewCollections: () => App.navigate('collections'),

  deleteCollection: async (name) => {
    const msg = await Translator.t('Delete collection');
